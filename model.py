...
class MnistFullyCNNSmall(nn.Module):
    def __init__(self):
        super().__init__()
        # First block
        self.block1 = nn.Sequential(
...
class MnistFullyCNNSmaller(nn.Module):
    def __init__(self):
        super().__init__()
        # First block
        self.block1 = nn.Sequential(
...
class MnistFullyCNNSmaller8kParams(nn.Module):
    def __init__(self):
        super().__init__()
        # First block
        self.block1 = nn.Sequential(
        ...
        # Third block
        self.block3 = nn.Sequential(
            nn.Conv2d(16, 16, kernel_size=3, stride=1, padding=1), # 7x7 -> 7x7
            nn.ReLU(),
            nn.BatchNorm2d(16),
            nn.Dropout(p=0.05),
        )
...
class MnistFullyCNNTiny4kParams(nn.Module):
    def __init__(self):
        ...
        self.block1 = nn.Sequential(
            ...
            nn.BatchNorm2d(8),
            # nn.Dropout(p=0.05),
            nn.Conv2d(8, 8, kernel_size=3, stride=1, padding=1), # 28x28 -> 28x28
            nn.ReLU(),
            nn.BatchNorm2d(8),
            nn.Dropout(p=0.03),
            nn.MaxPool2d(2, 2)  # 28x28 -> 14x14
        ...
        # Second block
        self.block2 = nn.Sequential(
            nn.Conv2d(8, 12, kernel_size=3, stride=1, padding=1), # 14x14 -> 14x14
            nn.ReLU(),
            nn.BatchNorm2d(12),
            # nn.Dropout(p=0.05),
            nn.Conv2d(12, 12, kernel_size=3, stride=1, padding=1), # 14x14 -> 14x14
            nn.ReLU(),
            nn.BatchNorm2d(12),
            nn.Dropout(p=0.03),
            nn.MaxPool2d(2, 2),  # 14x14 -> 7x7
        ...
        # Third block
        self.block3 = nn.Sequential(
            nn.Conv2d(12, 16, kernel_size=3, stride=1, padding=1), # 7x7 -> 7x7
            nn.ReLU(),
            nn.BatchNorm2d(16),
            nn.Dropout(p=0.03),
        )
